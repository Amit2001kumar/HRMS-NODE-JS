...
const attachmentController = require("../controllers/timesheet.attachment.controller");
...
// upload document
router.post("/timesheet_document", upload.single("profile"), function (req, res) {
  ...
  if (!req.file) {
    ...
    console.log(req);
    var sql =
      "INSERT INTO `timesheet_attachment` (`name`, `type`, `size`, `path`, `originalname`,`document_name`,`employeeId`) VALUES ('" +
      req.file.filename +
      "', '" +
      req.file.mimetype +
      "', '" +
...
router.get('/:employeeId', attachmentController.findById);
